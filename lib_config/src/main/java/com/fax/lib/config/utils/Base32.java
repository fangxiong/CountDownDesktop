...
public class Base32 extends BaseNCodec {
    ...
    // some state be preserved between calls of encodeString() and decodeString().
    ...
    public Base32(final int lineLength, final byte[] lineSeparator, final boolean useHex) {
        ...
        if (lineLength > 0) {
            ...
            if (containsAlphabetOrPad(lineSeparator)) {
                final String sep = newString(lineSeparator);
     ...
     * with the data to decodeString, and once with inAvail set to "-1" to alert decoder that EOF has been reached. The "-1"
     ...
     * </p>
     *
     * @param in      byte[] array of ascii data to Base32 decodeString.
     ...
     * the data to encodeString, and once with inAvail set to "-1" to alert encoder that EOF has been reached, so flush last
     * remaining bytes (if not multiple of 5).
     * </p>
     *
     * @param in      byte[] array of binary data to Base32 encodeString.
     * @param inPos   Position to start reading data from.
     * @param inAvail Amount of bytes available from input for encoding.
     * @param context the context to be used
     */
